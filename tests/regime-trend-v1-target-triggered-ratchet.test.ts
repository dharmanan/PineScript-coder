...
describe("Regime Trend v1 target-triggered ratchet", () => {
  ...
  it("activates after target touch and applies the floor only on the next 5m candle", () => {
    const candidate = TARGET_TRIGGERED_RATCHET_CANDIDATES.find(
      (item) => item.id === "touch-1.50-lock-0.00"
    )!;
    const candles = map([
  ...
  it("does not retroactively apply a ratchet inside the activation candle", () => {
    ...
    const candles = map([candle(0, { high: 116, low: 89 })]);
  ...
  it("keeps the baseline trend exit when activated but the floor is not touched", () => {
    ...
    const candles = map([
      candle(0, { open: 100, high: 121, low: 95 }),
      candle(FIVE, { open: 102, high: 125, low: 101 }),
      candle(2 * FIVE, { open: 103, high: 130, low: 102 })
    ]);
    const baseline = trade();
    const result = applyTargetTriggeredRatchet(baseline, [], candles, candidate);
    expect(result.classification).toBe("ACTIVATED_BASELINE_EXIT");
    expect(result.trade.net_pnl).toBe(baseline.net_pnl);
  });
