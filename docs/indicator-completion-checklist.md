...
- [ ] Apply safe indicator risk lifecycle handling to every non-spot Indicator output with visual risk.
- [ ] Show LONG/SHORT labels only when a new visual trade is actually accepted.
- [ ] Fire LONG/SHORT alerts only when a new visual trade is actually accepted.
- [ ] Report accepted entries, rather than ignored repeat signals, in the dashboard.

## Remaining

- [ ] Keep panel and entry divergence aligned whenever Confirmed RSI divergence is enabled.
- [ ] Align Strategy output with the same confirmed divergence engine.
