...
import {
  PHASE3_CANDIDATES,
  phase3Score,
  preparePhase3Features,
  runPhase3Candidate
} from "./phase3-tools.mjs";
import {
  parseArchiveCsv,
  sha256File
...
function evaluateCandidate(candidate, symbolContexts, partition) {
  ...
  for (const [symbol, contexts] of Object.entries(symbolContexts)) {
    ...
    for (const context of contexts[partition.id]) {
      const result = runPhase3Candidate(context.candles, context.features, candidate, {
        symbol,
...
function validationGates(result) {
  const positiveSymbols = Object.values(result.by_symbol)
    .filter((metrics) => metrics.total_net_return_units > 0).length;
  const metrics = result.combined;
  const enoughTrades = metrics.closed_trades >= 80;
  const positiveNet = metrics.total_net_return_units > 0;
  const pf = (metrics.profit_factor ?? 0) >= 1.1;
  const symbols = positiveSymbols >= 2;
  const drawdown = metrics.max_drawdown_return_units < 0.75;
  return {
    at_least_80_trades: enoughTrades,
    positive_net: positiveNet,
    profit_factor_at_least_1_10: pf,
    positive_on_two_symbols: symbols,
    drawdown_below_0_75: drawdown,
    passed: enoughTrades && positiveNet && pf && symbols && drawdown
  };
}
...
function ledgerCsv(trades) {
  const fields = [
    "candidate_id", "symbol", "direction", "signal_timestamp", "entry_timestamp",
    "entry_fill", "entry_atr", "initial_stop", "target", "exit_timestamp",
    "exit_fill", "exit_reason", "net_return", "bars_held", "ambiguous_intrabar"
  ];
  return `${fields.join(",")}\n${trades.map((trade) =>
...
async function main() {
  ...
  for (const symbol of manifest.symbols) {
    ...
    for (const partition of Object.values(partitions)) {
      symbolContexts[symbol][partition.id] = partitionSegments(candles, partition).map((segment) => ({
        candles: segment,
        features: preparePhase3Features(segment)
      }));
    }
    console.log(`${symbol}: ${candles.length} complete 15m candles`);
  }

  console.log(`Evaluating ${PHASE3_CANDIDATES.length} trend-aligned development candidates...`);
  const development = PHASE3_CANDIDATES.map((candidate) => {
    const result = evaluateCandidate(candidate, symbolContexts, partitions.development);
    return { candidate, result, score: phase3Score(result.combined) };
  ...
  const report = {
    schema_version: 1,
    study_id: "rsi-divergence-trend-aligned-phase3-v1",
    generated_at: new Date().toISOString(),
    source: "checksum-verified Binance Spot 5m archives aggregated to complete 15m candles",
    candidate_count: PHASE3_CANDIDATES.length,
    selection_policy: "rank frozen trend-aligned candidates on development only; independently validate top ten",
    costs,
    partitions,
  ...
  await writeFile(
    join(resultDirectory, "phase3-study-report.json"),
    `${JSON.stringify(report, null, 2)}\n`,
    "utf8"
  );
  for (const finalist of validation) {
    await writeFile(
      join(resultDirectory, `phase3-finalist-${finalist.candidate.id}-validation-ledger.csv`),
  ...
  console.log(`Report written to ${join(resultDirectory, "phase3-study-report.json")}`);
