...
export function parseBinanceKline(row) {
  ...
  const numeric = Object.values(candle);
  if (numeric.some((value) => !Number.isFinite(value))) throw new Error("Kline contains a non-finite value");
  return candle;
}
...
export function validateCandles(candles, options = {}) {
  ...
  for (let index = 0; index < candles.length; index += 1) {
    ...
    if (candle.closeTime !== candle.openTime + intervalMs - 1) throw new Error(`Invalid close time at ${candle.openTime}`);
    ...
    if (index > 0) {
      ...
      else if (delta !== intervalMs) gaps.push({ after: candles[index - 1].openTime, before: candle.openTime, delta });
    }
  }

  if (duplicates.length > 0) throw new Error(`Duplicate candles: ${duplicates.join(",")}`);
  if (gaps.length > 0) throw new Error(`Missing candle intervals: ${JSON.stringify(gaps.slice(0, 5))}`);
