...
export function validateConfig(c: StrategyConfig): ValidationIssue[] {
  ...
  if (c.risk.takeProfitMode === "risk_reward" && c.risk.stopMode === "none") error("risk_reward.no_stop", "Risk/reward targets require a stop-loss definition.");
  if (c.outputMode === "indicator" && (c.risk.stopMode !== "none" || c.risk.takeProfitMode !== "none")) warn("indicator.risk_visual_only", "Indicator mode cannot place orders; risk settings must be rendered as visual levels or omitted.");
